...
// ─── 홈 탭 ──────────────────────────────────────────────────
function HomeTab({ onSearch, history, market }) {
  ...
  return (
    <div>
      ...
      <section style={{ marginBottom:28 }}>
        <div style={{ display:"flex", alignItems:"flex-end", gap:24, flexWrap:"wrap" }}>
          <div style={{ flex:1, minWidth:200 }}>
            <div style={{ fontSize:10, fontWeight:700, color:C.secondary, textTransform:"uppercase", letterSpacing:"0.2em", marginBottom:8 }}>오늘의 학습</div>
            <div style={{ display:"flex", alignItems:"baseline", gap:6 }}>
              <span style={{ fontSize:80, fontWeight:800, color:C.primary, fontFamily:"Manrope,sans-serif", lineHeight:1, letterSpacing:"-0.04em" }}>{history.length}</span>
              <span style={{ fontSize:20, fontWeight:700, color:C.onSurfaceVariant }}>/ 5 단어</span>
            </div>
            <p style={{ fontSize:14, color:C.onSurfaceVariant, marginTop:8 }}>
          ...
          <div style={{ width:280, background:C.surfaceLow, padding:20, borderRadius:16 }}>
            ...
            <div style={{ height:10, background:C.surfaceContainer, borderRadius:20, overflow:"hidden" }}>
              <div style={{ height:"100%", borderRadius:20, background:C.secondary, width:`${Math.min(100, history.length / 5 * 100)}%`, transition:"width 0.5s", boxShadow:"0 0 12px rgba(182,32,55,0.3)" }} />
            </div>
            <div style={{ display:"flex", justifyContent:"space-between", marginTop:10, fontSize:10, fontWeight:700, color:C.outline, textTransform:"uppercase" }}>
      ...
      {/* 추천 단어 그리드 */}
      <section style={{ marginBottom:28 }}>
        <h2 style={{ fontSize:20, fontWeight:800, color:C.primary, fontFamily:"Manrope,sans-serif", marginBottom:18 }}>오늘의 추천 단어</h2>
        <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:14 }}>
          {/* 오늘의 단어 (2칸) */}
          <div onClick={() => onSearch(wotd.term)} style={{ gridColumn:"span 2", background:C.surface, borderRadius:16, padding:28, boxShadow:"0 12px 32px rgba(11,28,48,0.06)", cursor:"pointer", position:"relative", overflow:"hidden", minHeight:180, border:`1px solid ${C.outlineVariant}`, transition:"transform 0.2s" }}
            onMouseEnter={e => e.currentTarget.style.transform = "scale(0.99)"}
            onMouseLeave={e => e.currentTarget.style.transform = "scale(1)"}>
            <div style={{ position:"absolute", top:0, right:0, width:150, height:150, background:C.surfaceLow, borderRadius:"0 0 0 100%", marginTop:-50, marginRight:-50 }} />
            ...
            <div style={{ display:"flex", alignItems:"center", gap:12, marginTop:20 }}>
              <button style={{ background:C.primary, color:"#fff", padding:"10px 20px", borderRadius:12, border:"none", fontWeight:700, fontSize:13, cursor:"pointer" }}>
                지금 배우기 →
              </button>
          ...
          {/* 사이드 카드 */}
          <div onClick={() => onSearch(POPULAR[2])} style={{ background:C.surfaceLow, borderRadius:16, padding:22, cursor:"pointer", border:`1px solid ${C.outlineVariant}`, transition:"transform 0.2s" }}
          ...
          {/* 챌린지 배너 */}
          <div style={{ gridColumn:"span 3", background:C.primary, borderRadius:16, padding:22, display:"flex", justifyContent:"space-between", alignItems:"center", flexWrap:"wrap", gap:14 }}>
            <div>
            ...
            <div style={{ display:"flex", flexWrap:"wrap", gap:8 }}>
              {POPULAR.slice(3, 7).map(w => (
                <button key={w} onClick={e => { e.stopPropagation(); onSearch(w); }}
                  style={{ padding:"8px 14px", borderRadius:20, border:"1.5px solid rgba(255,255,255,0.3)", background:"transparent", color:"#fff", fontSize:12, fontWeight:600, cursor:"pointer" }}>
                  {w}
                </button>
              ))}
            </div>
      ...
      <section style={{ marginBottom:28 }}>
        ...
        <div style={{ background:C.surface, borderRadius:16, padding:18, overflowX:"auto", boxShadow:"0 4px 16px rgba(11,28,48,0.04)", border:`1px solid ${C.outlineVariant}` }}>
          <div style={{ display:"flex", gap:10, minWidth:"max-content" }}>
            {days.map((d, i) => (
              <div key={d} style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:6 }}>
                <div style={{ width:44, height:44, borderRadius:10, background: i === todayIdx ? C.secondary : i < todayIdx ? C.primaryFixed : C.surfaceHigh }} />
        ...
        <section>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(130px,1fr))", gap:10 }}>
            {[
              ...
              <div key={label} style={{ background:C.surface, borderRadius:14, padding:"16px 18px", border:`1px solid ${C.outlineVariant}`, boxShadow:"0 4px 12px rgba(11,28,48,0.04)" }}>
...
// ─── 단어 탭 ────────────────────────────────────────────────
function VocabTab({ onSearch, result, loading, error, query, setQuery, history }) {
  return (
    <div>
      ...
      <div style={{ display:"flex", gap:10, marginBottom:18 }}>
        <div style={{ flex:1, position:"relative" }}>
          <span style={{ position:"absolute", left:14, top:"50%", transform:"translateY(-50%)", color:C.outline, fontSize:20 }}>🔍</span>
          <input value={query} onChange={e => setQuery(e.target.value)} onKeyDown={e => e.key === "Enter" && onSearch(query)}
            ...
            style={{ width:"100%", paddingLeft:44, paddingRight:16, paddingTop:14, paddingBottom:14, background:C.surfaceLow, border:`1.5px solid ${C.outlineVariant}`, borderRadius:14, fontSize:14, color:C.onSurface, outline:"none", boxSizing:"border-box" }}
            onFocus={e => e.target.style.borderColor = C.primary}
            onBlur={e => e.target.style.borderColor = C.outlineVariant}
          />
        </div>
        <button onClick={() => onSearch(query)} disabled={loading}
          style={{ padding:"14px 22px", borderRadius:14, border:"none", cursor: loading ? "default" : "pointer", background: loading ? C.outline : C.primary, color:"#fff", fontWeight:700, fontSize:14, whiteSpace:"nowrap" }}>
          {loading ? "분석 중..." : "검색"}
        </button>
      </div>
        ...
        <div style={{ display:"flex", flexWrap:"wrap", gap:8 }}>
          {POPULAR.map(w => (
            <button key={w} onClick={() => onSearch(w)}
              style={{ padding:"7px 14px", borderRadius:20, cursor:"pointer", fontSize:12, fontWeight:600, border:`1.5px solid ${C.outlineVariant}`, background:"transparent", color:C.onSurfaceVariant, transition:"all 0.15s" }}
              onMouseEnter={e => { e.currentTarget.style.borderColor = C.primary; e.currentTarget.style.color = C.primary; e.currentTarget.style.background = C.primaryFixed; }}
              onMouseLeave={e => { e.currentTarget.style.borderColor = C.outlineVariant; e.currentTarget.style.color = C.onSurfaceVariant; e.currentTarget.style.background = "transparent"; }}>
              {w}
            </button>
          ))}
        </div>
      ...
      {loading && (
        <div style={{ background:C.surface, borderRadius:16, padding:"48px 24px", textAlign:"center", boxShadow:"0 4px 16px rgba(11,28,48,0.06)", border:`1px solid ${C.outlineVariant}` }}>
          <div style={{ fontSize:36, display:"inline-block", marginBottom:14, animation:"spin 1s linear infinite" }}>⚙️</div>
      ...
      {result && !loading && (
        <div style={{ animation:"fadeIn 0.4s ease" }}>
          <div style={{ background:C.surface, borderRadius:16, padding:28, marginBottom:16, boxShadow:"0 12px 32px rgba(11,28,48,0.08)", border:`1px solid ${C.outlineVariant}` }}>
            ...
            <div style={{ marginBottom:18 }}>
              <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:10 }}>
                <div style={{ width:28, height:28, borderRadius:"50%", background:C.primaryContainer, display:"flex", alignItems:"center", justifyContent:"center", fontSize:14 }}>🧠</div>
              ...
              <div style={{ background:C.surfaceLow, borderRadius:12, padding:"16px 18px", borderLeft:`3px solid ${C.primary}`, fontSize:14, color:C.onSurface, lineHeight:1.8 }}>
                {result.simple}
              </div>
            </div>
            ...
            <div>
              ...
              <div style={{ display:"flex", flexWrap:"wrap", gap:8 }}>
                {result.related.map(w => (
                  <button key={w} onClick={() => onSearch(w)}
                    style={{ padding:"8px 14px", borderRadius:20, border:`1.5px solid ${C.primary}`, background:"transparent", color:C.primary, fontSize:12, fontWeight:700, cursor:"pointer" }}
          ...
          <div style={{ display:"flex", flexWrap:"wrap", gap:8 }}>
            {history.map(h => (
              <button key={h} onClick={() => onSearch(h)}
                style={{ padding:"7px 14px", borderRadius:20, background:C.surfaceContainer, border:`1px solid ${C.outlineVariant}`, color:C.onSurfaceVariant, fontSize:12, fontWeight:600, cursor:"pointer" }}>
                {h}
              </button>
            ))}
          </div>
...
// ─── 분석 탭 ────────────────────────────────────────────────
function AnalysisTab({ history, market }) {
  return (
    <div>
      <section style={{ background:C.primary, borderRadius:20, padding:28, marginBottom:24, position:"relative", overflow:"hidden" }}>
        <div style={{ position:"absolute", top:0, right:0, width:200, height:200, background:"rgba(182,32,55,0.2)", borderRadius:"50%", filter:"blur(60px)", marginRight:-80, marginTop:-80 }} />
        <div style={{ position:"relative" }}>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", flexWrap:"wrap", gap:16 }}>
            <div>
              ...
              <div style={{ fontSize:64, fontWeight:800, color:"#fff", fontFamily:"Manrope,sans-serif", lineHeight:1, letterSpacing:"-0.04em", marginTop:12 }}>{history.length * 20}</div>
      ...
      {history.length > 0 && (
        <section style={{ marginBottom:24 }}>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
            {history.map((term, i) => (
              <div key={term} style={{ background:C.surface, borderRadius:14, padding:"16px 20px", display:"flex", justifyContent:"space-between", alignItems:"center", border:`1px solid ${C.outlineVariant}`, boxShadow:"0 2px 8px rgba(11,28,48,0.04)" }}>
                <div style={{ display:"flex", alignItems:"center", gap:14 }}>
                  <div style={{ width:32, height:32, borderRadius:"50%", background:C.primaryFixed, display:"flex", alignItems:"center", justifyContent:"center", fontSize:12, fontWeight:800, color:C.primary }}>{i + 1}</div>
                  <div style={{ fontSize:15, fontWeight:700, color:C.primary, fontFamily:"Manrope,sans-serif" }}>{term}</div>
                </div>
                <span style={{ color:"#00C471", fontSize:20 }}>✅</span>
              </div>
            ))}
          </div>
        </section>
...
export default function Home() {
  ...
  const search = useCallback(async (term) => {
    ...
    try {
      ...
      setError("AI 설명을 불러오지 못했어요. 다시 시도해주세요.");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  return (
    <div style={{ background:C.bg, minHeight:"100vh", fontFamily:"Inter,-apple-system,sans-serif", color:C.onSurface }}>
      {/* 헤더 */}
      <header style={{ position:"fixed", top:0, width:"100%", zIndex:50, background:"rgba(248,249,255,0.88)", backdropFilter:"blur(20px)", boxShadow:"0 1px 0 rgba(11,28,48,0.07)" }}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"16px 24px", maxWidth:1100, margin:"0 auto" }}>
          <div style={{ display:"flex", alignItems:"center", gap:10 }}>
            ...
            <h1 style={{ fontFamily:"Manrope,sans-serif", fontWeight:800, fontSize:20, color:C.primary, letterSpacing:"-0.02em" }}>ECON INTEL</h1>
          </div>
      ...
      {/* 콘텐츠 */}
      <main style={{ paddingTop:72, paddingBottom:90, padding:"72px 24px 90px", maxWidth:1100, margin:"0 auto" }}>
      ...
      {/* 바텀 네비 */}
      <nav style={{ position:"fixed", bottom:0, width:"100%", zIndex:50, background:"rgba(248,249,255,0.93)", backdropFilter:"blur(24px)", boxShadow:"0 -8px 24px rgba(11,28,48,0.06)", borderRadius:"24px 24px 0 0" }}>
        <div style={{ display:"flex", justifyContent:"space-around", alignItems:"center", maxWidth:480, margin:"0 auto", height:72, padding:"0 16px" }}>
          {nav.map(({ id, icon, label }) => {
            const active = tab === id;
            return (
              <button key={id} onClick={() => setTab(id)} style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:3, padding:"8px 20px", borderRadius:14, border:"none", cursor:"pointer", background: active ? C.primary : "transparent", color: active ? "#fff" : C.onSurface, opacity: active ? 1 : 0.4, transition:"all 0.15s" }}>
